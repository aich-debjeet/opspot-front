"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var client_1 = require("../../../../services/api/client");
var session_1 = require("../../../../services/session");
var web3_wallet_service_1 = require("../../../blockchain/web3-wallet.service");
var WalletTokenAddressesComponent = /** @class */ (function () {
    function WalletTokenAddressesComponent(client, web3Wallet, cd, router, session) {
        this.client = client;
        this.web3Wallet = web3Wallet;
        this.cd = cd;
        this.router = router;
        this.session = session;
        this.receiverAddress = '';
        this.addresses = [];
        this.inProgress = false;
        this.editing = false;
    }
    WalletTokenAddressesComponent.prototype.ngOnInit = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.receiverAddress = this.session.getLoggedInUser().eth_wallet;
                        if (!this.receiverAddress)
                            this.editing = true;
                        return [4 /*yield*/, this.getAddresses()];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    WalletTokenAddressesComponent.prototype.getAddresses = function () {
        return __awaiter(this, void 0, void 0, function () {
            var onchainAddress, e_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.inProgress = true;
                        this.addresses = [
                            {
                                address: this.receiverAddress,
                                label: 'Receiver',
                                selected: true
                            },
                            {
                                label: 'OffChain',
                                address: 'offchain',
                                selected: true
                            }
                        ];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, this.web3Wallet.getCurrentWallet()];
                    case 2:
                        onchainAddress = _a.sent();
                        if (!onchainAddress)
                            return [2 /*return*/];
                        if (this.addresses[0].address == onchainAddress) {
                            this.addresses[0].label = 'OnChain & Receiver';
                            this.detectChanges();
                            return [2 /*return*/]; //no need to count twice
                        }
                        this.addresses.unshift({
                            'label': "OnChain",
                            'address': onchainAddress,
                            'selected': true
                        });
                        this.detectChanges();
                        return [3 /*break*/, 4];
                    case 3:
                        e_1 = _a.sent();
                        console.log(e_1);
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    WalletTokenAddressesComponent.prototype.enableEditing = function () {
        this.session.getLoggedInUser().eth_wallet = '';
        this.editing = true;
    };
    WalletTokenAddressesComponent.prototype.disableEditing = function () {
        this.receiverAddress = this.session.getLoggedInUser().eth_wallet;
        this.editing = false;
        //this.detectChanges();
    };
    WalletTokenAddressesComponent.prototype.detectChanges = function () {
        this.cd.markForCheck();
        this.cd.detectChanges();
    };
    WalletTokenAddressesComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'm-wallet-token--addresses',
            templateUrl: 'addresses.component.html',
            changeDetection: core_1.ChangeDetectionStrategy.OnPush
        }),
        __metadata("design:paramtypes", [client_1.Client,
            web3_wallet_service_1.Web3WalletService,
            core_1.ChangeDetectorRef,
            router_1.Router,
            session_1.Session])
    ], WalletTokenAddressesComponent);
    return WalletTokenAddressesComponent;
}());
exports.WalletTokenAddressesComponent = WalletTokenAddressesComponent;
//# sourceMappingURL=addresses.component.js.map